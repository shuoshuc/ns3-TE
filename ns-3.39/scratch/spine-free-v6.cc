 ...
 * 65 clusters. Inside each cluster, the switches are connected as a folded Clos
...
// Ipv6InterfaceContainer filled with interfaces.
// e.g., 'tor-up': {if1, if2, ...}
using StageInterfaceMap = std::map<std::string, Ipv6InterfaceContainer>;
...
// TE row format: <group_type, src, dst, dst prefix (Ipv6Address), group>.
using TERow = std::tuple<uint32_t, std::string, std::string, Ipv6Address,
                         std::vector<uint32_t>>;
...
NS_LOG_COMPONENT_DEFINE("SpinefreeV6");
...
TEImpl readTEImpl(const std::string &filename, int NUM_TOR,
  ...
  while (std::getline(te_file, row)) {
    std::vector<uint32_t> weight_vec, group;
    std::vector<std::string> parsed_row = readCSVRow(row);
    if (parsed_row.empty()) {
      continue;
    }
    ...
    for (uint32_t i = 0; i < weight_vec.size(); ++i) {
      ...
      for (uint32_t repeat = 0; repeat < weight_vec[i]; ++repeat) {
    ...
    std::string prefix = "2001:" + seglist[1].erase(0, 1) + ":1::";
    ...
    te.push_back({std::stoi(parsed_row[0]), parsed_row[1], parsed_row[2],
                  Ipv6Address(prefix.c_str()), group});
  }
  return te;
}
...
void wipeStaticRoutingTable(Ptr<Node> node,
                            const Ipv6StaticRoutingHelper &ipv6RoutingHelper) {
  Ptr<Ipv6StaticRouting> staticRouting =
      ipv6RoutingHelper.GetStaticRouting(node->GetObject<Ipv6>());
...
void installLocalAndDefaultRoute(
    Ptr<Node> node, const Ipv6StaticRoutingHelper &ipv6RoutingHelper,
...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::string> subscribed_routing_tables{"toy3-c1-ab1"};
  ...
  */
  ...
  LogComponentEnable("SpinefreeV6",
  ...
  std::map<std::string, std::pair<Ptr<Ipv6>, uint32_t>> globalInterfaceMap;
  ...
  for (int i = 0; i < NUM_CLUSTER; ++i) {
    ...
    Ipv6AddressHelper intraClusterAddress;
    std::string intraBaseIP = "2001:" + std::to_string(i + 1) + ":1::";
    // ToR up-facing interfaces are assigned address 10.{cluster id}.1.{tor id}
    intraClusterAddress.SetBase(Ipv6Address(intraBaseIP.c_str()),
                                Ipv6Prefix(64));
    Ipv6InterfaceContainer torUpIfs =
    ...
    // 2001:{cluster id}:1::{100 + tor id}
    intraClusterAddress.SetBase(Ipv6Address(intraBaseIP.c_str()),
                                Ipv6Prefix(64), Ipv6Address("::101"));
    Ipv6InterfaceContainer aggrDownIfs =
    ...
    // 2001:100::{cluster id}:{port id / 2 + 1}
    Ipv6AddressHelper dcnAddress;
    std::string startIP = "::" + std::to_string(i + 1) + ":1";
    dcnAddress.SetBase(Ipv6Address("2001:100::"), Ipv6Prefix(64),
                       Ipv6Address(startIP.c_str()));
    Ipv6InterfaceContainer dcnIfs =
  ...
  for (int i = 0; i < NUM_CLUSTER; ++i) {
    ...
    wipeStaticRoutingTable(aggr, ipv6RoutingHelper);
    installLocalAndDefaultRoute(aggr, ipv6RoutingHelper, false);
    ...
    for (uint32_t j = 0; j < tors.GetN(); ++j) {
      wipeStaticRoutingTable(tors.Get(j), ipv6RoutingHelper);
      installLocalAndDefaultRoute(tors.Get(j), ipv6RoutingHelper);
      ...
      Ipv6Address torAddr =
          globalInterfaceMap[tor_egress_port]
              .first->GetAddress(globalInterfaceMap[tor_egress_port].second, 1)
              .GetAddress();
      Ptr<Ipv6StaticRouting> staticRouting =
          ipv6RoutingHelper.GetStaticRouting(aggr->GetObject<Ipv6>());
      // Host routes are always /128.
      staticRouting->AddNetworkRouteTo(torAddr, Ipv6Prefix(128), aggr_if_id);
    }
  }
  ...
  for (const auto &te_row : te) {
    ...
    Ipv6Address dst_prefix = std::get<3>(te_row);
    std::vector<uint32_t> group = std::get<4>(te_row);
    ...
    Ptr<Ipv6StaticRouting> staticRouting =
        ipv6RoutingHelper.GetStaticRouting(node->GetObject<Ipv6>());
    /*
    staticRouting->AddNetworkRouteTo(dst_prefix, Ipv6Prefix(64), direct_if_id,
                                     group_type, group);
    */
    staticRouting->AddNetworkRouteTo(dst_prefix, Ipv6Prefix(64), direct_if_id,
                                     group);
  ...
  for (const TMRow &row : TM) {
    ...
    Ipv6Address dstAddr =
        globalInterfaceMap[dst + "-p1"]
            .first->GetAddress(globalInterfaceMap[dst + "-p1"].second, 1)
            .GetAddress();
  ...
  // Dump flow stats.
  if (verbose) {
    flowMonitor->SerializeToXmlFile(outPrefix + "SpinefreeV6.xml", true, true);
  }
  Simulator::Destroy();
